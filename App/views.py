...
def userlogin(request):
    if request.POST:
        user = User(account=request.POST['account'], pwd=request.POST['pwd'])
        return HttpResponse(user.user_login(), content_type='application/json')
...
def uploadVideo(request):
    if request.method == 'POST':
        ...
        #     fileSave = models.FileField(upload_to='./uploadFile/')
        #
        #     #
        #     # class Meta:
        #     #     managed = False
